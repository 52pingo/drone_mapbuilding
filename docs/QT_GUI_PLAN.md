...
> 实施状态（2026-08-18）：M1 与 M2 控制面已实现，完成 PySide6 主窗口、配置模型、
> 本地/WSL 动态自检、QProcess 编排、NED 航点编辑、结构化飞行遥测以及
> Hold/Resume/Safe Land 服务。连续 RGB/检测消息、实时轨迹绘制和三维点云仍按
> M3/M4 推进；GUI 大环线实飞是 M2 的剩余现场验收项。
